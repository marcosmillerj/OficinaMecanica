...
public class OrdemServicoService {
     ...
     * @param clienteService O serviço de clientes (para validar existência de
     * cliente por ID).
     * @param veiculoService O serviço de veículos (para validar existência de
     * veículo por ID). // Futuramente: @param servicoService O serviço de
     * serviços.
     */
    ...
    /**
     * Cria uma nova Ordem de Serviço no sistema. Realiza validações de
     * existência de cliente, veículo e mecânico.
     *
     ...
     * @throws IllegalArgumentException Se cliente, veículo ou mecânico não
     * forem encontrados.
     */
     ...
     * @throws IllegalArgumentException Se a OS não for encontrada ou o serviço
     * for nulo.
     * @throws IllegalStateException Se a OS não estiver em um status que
     * permite adição de serviços.
     */
     ...
     * @return A Ordem de Serviço atualizada.
     * @throws IllegalArgumentException Se a OS não for encontrada ou o serviço
     * for nulo ou não estiver na OS.
     * @throws IllegalStateException Se a OS não estiver em um status que
     * permite remoção de serviços.
     */
     ...
     *
     * @param idOs ID da Ordem de Serviço.
     * @return Um Optional contendo a Ordem de Serviço, ou vazio se não
     * encontrada.
     */
    ...
    public List<OrdemServico> buscarPorMecanicoEStatus(int mecanicoId, StatusOrdem status) {
        return ordemServicoRepository.listarOrdensPorMecanicoEStatus(mecanicoId, status);
    }
    ...
    public OrdemServicoRepository getRepository() {
     ...
     public boolean validarTransicaoStatusMecanico(StatusOrdem atual, StatusOrdem novo) {
        // Implementação das regras específicas
        switch (atual) {
            case AGUARDANDO_DIAGNOSTICO: return novo == StatusOrdem.EM_DIAGNOSTICO;
            case EM_DIAGNOSTICO: return novo == StatusOrdem.AGUARDANDO_LIBERACAO || novo == StatusOrdem.CANCELADA;
            case EM_EXECUCAO: return novo == StatusOrdem.AGUARDANDO_PAGAMENTO || novo == StatusOrdem.CANCELADA;
